...
M0 + M1 + M2：安全 Electron 基础、可持久化的手工知识图谱，以及独立的学习状态与证据投影。不要提前实现客观评估、Tutor Agent、文档抽取、RAG、同步或真实 LLM。
...
- 不得通过删除或弱化测试来掩盖故障。
- 当前 Definition of Done：图谱结构可编辑并持久化；循环先修关系被拒绝；学习证据可解释状态且不会被普通图谱保存清除；重启可恢复；安全边界成立；检查全绿；README 命令准确。
