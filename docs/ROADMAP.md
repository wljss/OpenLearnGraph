...
- **M7C — Trustworthy candidate routes（完成）**：概念数量和粒度采用更保守的默认值；AI 先修关系必须说明具体依赖并保留独立原文快照，审核界面用“先学 / 再学 / 为什么”降低判断负担；旧版无依据 AI 关系阻止直接写入，所有建议仍须用户最终确认。
- **M7D — Resilient generation and learning handoff（完成）**：renderer 通过窄 IPC 读取 main 中的真实批次进度，明确区分生成、出处核对和合并阶段；重复点击受阻，主动取消丢弃未完成结果，失败保留已确认范围供安全重试；路线写入后可直接开始第一个已解锁且有内容的概念。
- **学习进度与成果反馈（完成）**：图谱总进度坚持 Evidence 口径，区分客观诊断确认与主观自评掌握，并把题库准备度独立展示；侧栏提供跨图谱进度概览，学习、练习和诊断完成页说明实际成果、解锁变化与下一步，不把导入、生成或形成性练习包装成掌握。
- **首次上手与任务式引导（完成）**：全新用户可从导入资料、手工建图或可删除示例开始；六项任务直接跳转真实功能、可跨重启继续并可随时重开。快速介绍可以重复查看而不重置真实任务或学习数据，已有示例只会继续打开；老用户升级不强制弹出。引导完成度与知识掌握进度严格分离。
