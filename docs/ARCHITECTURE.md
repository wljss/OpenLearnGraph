...
`npm.cmd run smoke:documents` 在隔离临时资料库中启动正式 EXE，经真实 preload/IPC/main/SQLite 路径检查 PDF、EPUB、Markdown、TXT 的预览、确认、重复拦截、异常反馈、重启读取、长资料定位及资料库界面打开，同时覆盖原文选区创建候选、关系审核、原子写入和重启后审核记录保留；用 loopback DevTools 临时替换系统文件选择器，不向生产应用添加测试专用接口。其样本是合成样本，复杂中文 PDF/EPUB 仍须用授权的真实资料进行人工核对。
...
后续 M7B 的 DeepSeek 等云端 provider 只能产出经共享 schema 验证的候选项，不能越过这条审核边界或直接写 SQLite。发送前必须让用户看到资料范围，API Key 不进入 renderer、数据库、日志或 Git。
