...
每批响应必须先通过运行时 JSON schema、概念/关系唯一性、引用完整性、先修引用和无环检查；每条引用还必须逐字存在于本批授权片段，再由 main 还原为完整章节的 Unicode 码点区间。全部批次成功后按规范化名称合并同名概念，并丢弃合并后产生的重复、自循环或冲突关系；最后通过一次 repository 调用原子写入候选区。请求支持主动取消和每批 90 秒超时，HTTP/网络错误转换为可执行的中文提示。`ai_generation_runs` 仅审计 provider、模型、完整章节范围、总字符数、汇总 token 数、结果数量、状态和错误，不保存正文、提示词、原始响应或密钥。
