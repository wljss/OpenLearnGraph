...
M7C 将候选关系也视为需要独立出处的审计对象。AI 关系必须包含面向学习者的原因和一个本批 `sourceId`；main 从该编号重建资料、章节、Unicode 码点区间和原文快照，再与关系一起持久化。审核界面直接显示“先学 / 再学 / 为什么”，原文依据按需展开。M7C 以前生成且缺少原因或出处的 AI 关系不会被静默删除，但会形成写入阻断，必须移除或重新生成；手工关系不伪装成有 AI 依据的关系。
...
概念生成默认收敛到 3–8 个核心知识点。先修关系采用保守准则：只有目标定义、步骤或推导会直接使用来源概念时才建议关系；同页出现、章节顺序、对比、组成、模型配置或成本分析不能单独证明先修。允许返回零条关系，不要求候选图连通。原因与出处提高可审核性，但不被当作客观真值或掌握证据，正式图谱仍须用户确认。
