...
M6A 的文件选择、读取、散列和解析只在 main 执行。renderer 只能请求打开受限扩展名的系统选择器，收到的是不含本机路径的结构化预览和短期随机 token；确认接口只接受 token 与经过 Zod 校验的可编辑元数据。服务在确认时重新核对文件大小、修改时间与内容 SHA-256，并用 SHA-256 唯一约束阻止内容相同的重复资料。预览 token 仅保存在内存中、30 分钟后失效，关闭预览时主动释放。
...
`npm.cmd run smoke:documents` 在隔离临时资料库中启动正式 EXE，经真实 preload/IPC/main/SQLite 路径检查 PDF、EPUB、Markdown、TXT 的预览、确认、重复拦截、异常反馈、重启读取、长资料定位及资料库界面打开，同时覆盖原文选区创建候选、关系审核、原子写入、重启后审核记录保留，以及 AI Key 安全保存和正文上传确认界面；用 loopback DevTools 临时替换系统文件选择器，不向生产应用添加测试专用接口，也不会发起真实云端请求。其合成样本之外，复杂中文 PDF/EPUB 仍须用授权的真实资料进行人工核对。
...
M7B 的 DeepSeek 只能产出经共享 schema 验证的候选项，不能越过这条审核边界或直接写正式图谱。通过验证的整批结果由 repository 以 `AI` 来源和实际模型标记写入待审核区；任一候选或关系不合格时整批拒绝，不留下部分结果。
...
用户点击生成后，`AiService` 从 SQLite 重新读取所选章节并创建 10 分钟有效的预览 token；renderer 展示资料、章节、字符数和摘要，只有用户明确勾选同意后才可继续。确认时 main 再次读取相同章节，只发送该范围的正文，不发送本机路径、其他章节、学习状态或 Evidence。

响应必须先通过运行时 JSON schema、概念/关系唯一性、引用完整性、先修引用和无环检查；每条引用还必须逐字存在于对应章节，再由 main 计算 Unicode 码点区间。请求支持主动取消和 90 秒超时，HTTP/网络错误转换为可执行的中文提示。`ai_generation_runs` 仅审计 provider、模型、章节范围、字符数、token 数、结果数量、状态和错误，不保存正文、提示词、原始响应或密钥。
