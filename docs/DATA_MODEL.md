...
示例图谱 ID 只保存在该单行表中。创建示例时，图谱、3 个节点、2 条先修关系和 6 道预置 `BOTH` 题在一个事务中写入；重复创建会返回同一示例。只有 `NOT_STARTED` 会在首次创建示例时进入 `IN_PROGRESS`，已经完成或跳过的引导不会因重建示例而回退。专用删除操作只接受此已记录 ID，删除图谱时由外键级联清理其题目、会话和 Evidence，并将 `sample_graph_id` 置空。
